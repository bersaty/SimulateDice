...
public class LoadUtil
{
    ...
    public static Dice loadDiceObj(String fname, Resources r,Context context)
    {
        //加载后物体的引用
        Dice lo=null;
        ...
        //原始纹理坐标
        ArrayList<Float> altexCoord=new ArrayList<Float>();

        //最终纹理坐标
        ArrayList<Float> altexResultCoord=new ArrayList<Float>();
        ...
        try
        {
            ...
            while((temps=br.readLine())!=null)
            {
                ...
                {//此行为顶点坐标
                    ...
                    alv.add(Float.parseFloat(tempsa[1])/10.0f);
                    alv.add(Float.parseFloat(tempsa[2])/10.0f);
                    alv.add(Float.parseFloat(tempsa[3])/10.0f);
                }
                //纹理坐标
                else if(tempsa[0].trim().equals("vt")){
                    altexCoord.add(Float.parseFloat(tempsa[1]));
                    altexCoord.add(-1*Float.parseFloat(tempsa[2]));
                }
                ...
                {//此行为三角形面
                    ...
                    indexTex[0]=Integer.parseInt(tempsa[1].split("/")[1])-1;
                    float x00=altexCoord.get(2*indexTex[0]);
                    float y00=altexCoord.get(2*indexTex[0]+1);
                    altexResultCoord.add(x00);
                    altexResultCoord.add(y00);

                    //计算第1个纹理的索引，并获取此顶点的XY坐标
                    indexTex[1]=Integer.parseInt(tempsa[2].split("/")[1])-1;
                    float x11=altexCoord.get(2*indexTex[1]);
                    float y11=altexCoord.get(2*indexTex[1]+1);
                    altexResultCoord.add(x11);
                    altexResultCoord.add(y11);

                    //计算第2个纹理的索引，并获取此顶点的XY坐标
                    indexTex[2]=Integer.parseInt(tempsa[3].split("/")[1])-1;
                    float x22=altexCoord.get(2*indexTex[2]);
                    float y22=altexCoord.get(2*indexTex[2]+1);
                    altexResultCoord.add(x22);
                    altexResultCoord.add(y22);
            ...
            //生成纹理坐标
            int sizeuv=altexResultCoord.size();
            float[] uv=new float[size];
            for(int i=0;i<sizeuv;i++)
            {
                uv[i]=altexResultCoord.get(i);
            }


            //创建3D物体对象
            lo=new Dice(context,vXYZ,nXYZ,uv);
        }
        catch(Exception e)
        {
